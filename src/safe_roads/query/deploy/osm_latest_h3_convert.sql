-- 0) Start clean
DROP TABLE IF EXISTS osm_latest_h3;

-- 1) Create an EMPTY table with same cols + h3
CREATE TABLE osm_latest_h3 AS
SELECT o.*, NULL::h3index AS h3
FROM osm_data_latest o
WHERE FALSE;   -- <-- key: defines schema only, no rows

--LINES → corridor buffer → H3 cells (r=12)
INSERT INTO osm_latest_h3
...
JOIN LATERAL h3_polygon_to_cells_experimental(
  ST_Buffer(t.geog_4326, p.buf_m)::geometry,  -- corridor in meters
  p.r,
  'overlapping'
) AS h3c(h3) ON TRUE
...
-- POINTS / MULTIPOINTS → dump → H3 cell (r=12)
INSERT INTO osm_latest_h3
