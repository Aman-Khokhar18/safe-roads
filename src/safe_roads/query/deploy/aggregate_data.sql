-- drop first (optional)
DROP TABLE IF EXISTS public.osm_deploy_latest;

-- create from the single WITH…SELECT statement
CREATE TABLE public.osm_deploy_latest AS
WITH
-- 1) Per-H3 aggregation from raw OSM rows
per_cell AS (
  SELECT
    h3,
  ...
  FROM public.osm_latest_ml_features
  GROUP BY h3
),

-- proportions to de-bias by number of OSM rows
per_cell_with_shares AS (
  SELECT
    p.*,
...
-- 2) One "dimension" row per H3 (carry res_11, borough, etc.) — weather removed
event_dim AS (
...
-- 3) res_11 context by averaging per-H3 features within each res_11
res11_context AS (
  SELECT
    d.res_11,

    COUNT(*)::int                   AS r11_n_cells,
    AVG(f.n_osm_features)::float    AS r11_mean_n_osm_features,
    SUM(f.n_osm_features)::int      AS r11_total_osm_features,

    /* numeric context (means across H3 cells) */
    ...
    /* safety-relevant means (counts per H3 cell) */
    ...
    /* sidewalks / cycling (mean counts per H3 cell) */
  ...
  FROM per_cell_with_shares f
  JOIN event_dim d USING (h3)
  GROUP BY d.res_11
)

-- 4) Final one-row-per-H3 result becomes the table (no weather fields)
SELECT
  d.h3,
  d.res_11,
  d.year,
  ...
  /* per-H3 distributions & categorical counts */
...
FROM event_dim d
JOIN per_cell_with_shares f USING (h3)
LEFT JOIN res11_context r11 USING (res_11);
