...
DROP TABLE IF EXISTS public.osm_h3_ml_features;

CREATE TABLE public.osm_h3_ml_features AS
WITH base AS (
  SELECT
    h3,
    "geometry"        AS geometry,   -- carry original geometry
    geom_4326,                       -- carry WGS84 geometry if present
...
SELECT
  ...
  geom_4326,           -- include geom_4326 from source
  highway_class,
...
-- Helpful indexes
CREATE INDEX IF NOT EXISTS idx_osm_h3_ml_features_h3
  ON public.osm_h3_ml_features(h3);

CREATE INDEX IF NOT EXISTS idx_osm_h3_ml_features_highway_class
  ON public.osm_h3_ml_features(highway_class);

-- Spatial indexes (optional but recommended)
CREATE INDEX IF NOT EXISTS idx_osm_h3_ml_features_geom_gist
  ON public.osm_h3_ml_features USING GIST (geometry);

CREATE INDEX IF NOT EXISTS idx_osm_h3_ml_features_geom4326_gist
  ON public.osm_h3_ml_features USING GIST (geom_4326);
