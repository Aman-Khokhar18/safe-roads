-- drop first (optional)
DROP TABLE IF EXISTS public.collision_dataset;

-- create from the single WITH…SELECT statement
CREATE TABLE public.collision_dataset AS
WITH
...
event_dim AS (
  SELECT DISTINCT ON ("collisionID")
    "collisionID",
    h3,
    res_11,
    event_year,
    "Borough",
    COALESCE(borough_norm, public.normalize_london_borough("Borough")) AS borough_norm,
    "Easting",
    "Northing",
    date,
    "Time",
  ...
  ORDER BY
    "collisionID",
    (event_dt IS NULL), event_dt DESC,
...
res11_context AS (
  ...
  JOIN event_dim d USING ("collisionID")
  GROUP BY d.res_11
)
...
JOIN per_collision_with_shares f
  ON d."collisionID" = f."collisionID"
