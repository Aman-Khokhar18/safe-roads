...
FROM london_osm_sample d
LEFT JOIN LATERAL (
  SELECT *
  FROM weather_hourly w
  WHERE w.datetime BETWEEN d.ts - interval '6 hours'
                               AND d.ts + interval '6 hours'
  ORDER BY ABS(EXTRACT(EPOCH FROM (w.datetime - d.ts)))  -- nearest first
  LIMIT 1
) AS w ON TRUE;
