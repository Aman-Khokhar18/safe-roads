DROP TABLE IF EXISTS roads_features_negatives;
CREATE TABLE roads_features_negatives AS
WITH normed AS (
  SELECT
    r.*,
...
SELECT
  ...
  -- day of month (cyclic within month) using timestamp present in r
  CASE
    WHEN year_i IS NOT NULL AND month_i IS NOT NULL AND day_i IS NOT NULL THEN
      sin(
        2 * pi() * (
